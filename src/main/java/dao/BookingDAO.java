...
public class BookingDAO extends DBConnection{
    ...
    public String addFlightBooking(String standFor, FlightBooking flightBooking) {
        ...
        try {
            ...
            seatNumber++;
            newSeatNumber = standFor + String.format("%03d", seatNumber);
    ...
    // tạo hàm get booking by email
    private static final String FIND_BOOKINGS_BY_EMAIL_SQL = "SELECT * FROM booking.Flight_Booking where email_user = ?";
            public List<FlightBooking> findBookingsByUser(String email) {
                List<FlightBooking> bookings = new ArrayList<>();
        
                try (Connection connection = getConnection();
                    PreparedStatement preparedStatement = connection.prepareStatement(FIND_BOOKINGS_BY_EMAIL_SQL)) {
